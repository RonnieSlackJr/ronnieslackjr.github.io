...
//if n undefined or not number, return array[0]
//otherwise return first n number of items
...
//if not array, return []
//if not number or undefined, return 1st array[array.length-1]
//check for negatives
//otherwise, return last n number of elements in array
    if (!Array.isArray(array) || n < 0) return [];
...
//1. check if collection is an array
//2. loop through array
//3. perform action on the 3 arguments: property's value, index/key, collections
//4. check if collection is an object, or just assume it is
//5. loop through object, perfrom action on 3 arguments
...
//2 return [i] of array that has first occurence of value
//3 if value not in array, return -1
...
// // loop through array
// // action on each element in array. pass  array, index, and array
// //put elements that return true into a new array
// //return THAT array
    var areTrue = [];
...
//create empty array
//call filter
//function in filter should be if elements return false, use !
    var areFalse = [];
...
//two empty arrays, one to catch truthy, one for falsey, one to hold them
// run filter, put passing values in truthy array
// run reject, put those values in falsey array
//set truthy values to array[0], falsey array[1];
    var areTrue = [];
    var areFalse = [];
    var partitioned= [];
    _.filter(array, function(value, index, array){
...
//_.each for collection and function with the 3 params
//push return value to empty array
// repeat for object
    var mapResults = [];
